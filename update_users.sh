...
if [[ "$#" -eq "0" ]]; then
    ...
    if [[ $REVOKE_ALL_CLIENT_CERTIFICATES =~ ^[Y]$ ]]; then
...
for ovpn_filename in *.ovpn; do
    ovpn_users=("${ovpn_users[@]}" "${ovpn_filename%.*}")
done

# Revoke excess client certificates
for ovpn_user in ${ovpn_users[@]}; do
    if is_in_array "${ovpn_user}" "${USERNAMES}"; then
